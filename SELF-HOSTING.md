...
- **With no forge:** run the CLI over each folder of zips (see [`publish-action/README.md`](publish-action/README.md)). It writes the zips and their `index.json` as static files that any internal web server can serve. `registry.json` is a short file you keep by hand, listing the URL of each `index.json`.
